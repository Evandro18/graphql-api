...
// Db is our database struct used for intecting with the database
type Db struct {
	*sql.DB
}
...
func (d *Db) GetUsersByName(name string) [User]  {
	// Prepare query, takes a name arguments, protects from sql injection
	...
	// Copy the column from row into the values pointed at by r (User)
	users := []User{}
	for rows.Next() {
		err = rows.Scan(
			&r.ID,
